...
Scores and tiers a target account list across four criteria — ICP
fit, buying readiness, revenue potential, and relationship warmth —
and assigns each account to one of four tiers: Strategic, Full ABM,
Light ABM, or Programmatic. Accepts pasted account lists or CSV
files dropped into inputs/accounts_to_score.csv.

Reads all context files before scoring: icp.md for segment
definitions and persona fit, market.md for revenue potential
calibration, competitors.md to flag displacement opportunities,
proof_points.md to identify accounts that resemble named customers,
and the deep-dive files for trigger event and buying committee
intelligence. Scans outputs/ for prior Trigger Monitor and
Competitor Ad Intelligence reports — accounts with active signals
in those reports receive higher Buying Readiness scores automatically.

Displays the full scoring rubric before producing scores. For each
account: a score on all four criteria with a one-sentence rationale
for each, a total score out of 100, a confidence rating, and a
competitor flag. Applies relative tiering automatically if the
absolute thresholds produce a skewed distribution, and flags
clearly when this happens.

For Strategic and Full ABM accounts: a recommended next action and
a ready-to-use orchestrator brief the user can paste directly to
generate the outreach campaign. For Light ABM and Programmatic
accounts: abbreviated entries with treatment guidance.

Produces two output files: a prioritised account file with full
scoring rationale and next actions, and a separate Trigger Monitor
feed file with tier assignments that adjusts search depth per
account when present. The Trigger Monitor runs at full depth for
all accounts when no tier data exists — tier data is optional
context, not a dependency.

### A/B Test Tracker

Tracks all A/B tests run across copy, audience, landing page,
email, and channel formats. Reads from existing outputs/ files —
Performance Grader reports, campaign copy files, Variant Scorer
outputs, CRO briefs, email sequence files, LinkedIn strategy files,
and paid social audience files — and accepts manual entry for tests
run but not yet recorded in any output file.

Produces a full test log organised by persona and segment, with
each entry showing what was tested, which variant won, by how much,
the confidence level of the result, and whether the Variant Scorer's
pre-launch prediction matched the live outcome. Flags every instance
where the prediction did not match and categorises the discrepancy
type — awareness level mismatch, pain point mismatch, brand voice
overcorrection, or sample size issue — with an implication for how
future scoring should be weighted for that persona and channel.

Pattern analysis identifies what is consistently winning or losing
across multiple tests for each persona. Patterns require evidence
from two or more tests — single-test findings are labelled as
hypotheses. Overall patterns surface the strongest performing angle
across all tests, the most tested variable, the least tested areas,
and any brand voice deviations that are winning tests despite
conflicting with guidelines.

Produces a prioritised list of up to five recommended next tests,
each with a ready-to-use orchestrator brief. Recommendations are
grounded in specific gaps, patterns needing confirmation, or
Variant Scorer discrepancies — never speculative.

Accumulates history across runs — each saved tracker file is loaded
on the next run so patterns strengthen over time. Never updates
context files — surfaces findings for human review only.
