...
### OPTIONAL — ABM Tier Data

Before running searches, check
outputs/ for any file matching
the pattern abm_tiers_*.md

If found:
  Read the file and note which
  accounts are in each tier.
  Adjust search depth as follows:

  Strategic accounts:
    Run all 5 searches as normal.
    Produce full outreach intelligence
    for every signal found regardless
    of signal rating — Hot, Warm, and
    Monitor signals all get full
    treatment.

  Full ABM accounts:
    Run all 5 searches as normal.
    Produce full outreach intelligence
    for Hot signals only. Warm signals
    get a brief summary. Monitor
    signals are noted but not expanded.

  Light ABM accounts:
    Run 3 searches per account —
    searches 1, 2, and 4 from the
    standard search sequence.
    Produce outreach intelligence
    only for Hot signals.

  Programmatic accounts:
    Run 1 search per account —
    search 1 only.
    Note any Hot signal found but
    do not produce full outreach
    intelligence. Flag for human
    review and suggest upgrading
    to Light ABM if a Hot signal
    is confirmed.

If no abm_tiers_*.md file is found:
  Run all accounts at full depth
  as normal. No change to behavior.
  Do not mention the absence of
  tier data in the output — just
  proceed as normal.

Display at the start of the run
only if tier data is found:

---
ABM tier data detected: [filename]
Applying tiered search depth.
Strategic: [count] accounts —
  full coverage, all signals
Full ABM: [count] accounts —
  full coverage, Hot signals only
Light ABM: [count] accounts —
  reduced coverage (3 searches)
Programmatic: [count] accounts —
  signal-only coverage (1 search)
---

Display nothing extra if no tier
data is found — just run normally.
